...
// OPTION A — Headline dramatically larger. Clear size jump between headline and body copy.

export function Hero() {
  return (
    <section id="top" className="relative overflow-hidden bg-background">
      {/* Mobile */}
      <div className="px-5 pt-16 pb-6 sm:px-8 sm:pt-20 lg:hidden">
        <div className="mx-auto max-w-[1400px]">
          <Reveal className="text-center">
            <RevealItem
              as="div"
              blur
              className="font-serif text-3xl leading-[1.15] font-medium text-balance text-foreground sm:text-7xl"
            >
          ...
          <div className="mt-3 grid grid-cols-2 items-stretch gap-3 sm:mt-6 sm:gap-6">
            <Reveal className="flex items-center text-left">
              <RevealItem className="text-[1.05rem] leading-[1.85] text-muted-foreground sm:text-lg sm:leading-relaxed">
                <p>
          ...
          <Reveal stagger className="mt-3 grid grid-cols-2 gap-2 border-t border-border pt-3 sm:mt-8 sm:flex sm:flex-wrap sm:items-center sm:gap-x-6 sm:gap-y-2 sm:pt-6">
            {socialProof.map((item) => (
              <RevealItem key={item} className="text-[0.65rem] leading-snug tracking-wide text-muted-foreground sm:text-sm">
                {item}
              </RevealItem>
            ))}
          </Reveal>
      ...
      {/* Desktop */}
      <div className="hidden lg:flex lg:min-h-[85vh] lg:items-center lg:pt-28 lg:pb-16">
        <div className="mx-auto flex w-full max-w-7xl items-center gap-24 px-8 xl:gap-28 xl:px-12">
          <div className="flex flex-col justify-center flex-1">
            <Reveal stagger className="max-w-3xl">
              <RevealItem as="div" blur
                className="font-serif text-[3.5rem] leading-[1.1] font-medium text-balance text-foreground xl:text-[4.5rem]">
                <h1>Look As Vibrant As You Feel</h1>
              </RevealItem>
              ...
              <RevealItem className="mt-10 flex flex-nowrap items-center gap-x-8 whitespace-nowrap border-t border-border pt-8">
                {socialProof.map((item) => (
                  <span key={item} className="text-sm tracking-wide text-muted-foreground">{item}</span>
                ))}
              </RevealItem>
            </Reveal>
          </div>
          ...
          <Reveal blur delay={0.15} className="relative shrink-0">
            <div className="relative h-[450px] w-[380px] overflow-hidden rounded-3xl shadow-[0_0_1px_rgba(0,0,0,0.05),0_2px_5px_rgba(0,0,0,0.08),0_8px_16px_rgba(0,0,0,0.08),0_16px_32px_rgba(0,0,0,0.06),0_24px_48px_rgba(0,0,0,0.04)]">
              <Image src="/images/dr-sophia-laurent.png" alt="Dr. Sophia Laurent" fill priority sizes="380px" className="object-cover" />
            </div>
          </Reveal>
        </div>
      </div>
